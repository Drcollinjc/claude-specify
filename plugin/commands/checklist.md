...
4. **Product Thesis Awareness**: Load `.specify/memory/product-principles.md` (the product thesis). During checklist generation, perform a **full thesis check** against ALL thesis principles. This is an inline check — thesis principles can inform checklist domains.
   ...
   **Thesis check behavior**:
   - For each thesis principle, assess whether it suggests a checklist domain worth covering
   - If a thesis principle is particularly relevant to this feature (e.g., §2/§10 for AI features → explainability checklist items, §6 for features near the boundary → scope checklist items):
     - Include thesis-informed items in the generated checklist (e.g., "Are explanation mechanism requirements defined for all AI outputs? [Thesis §2/§10]")
   - If no thesis concerns are relevant: proceed without thesis-specific items
...
6. **Generate checklist** - Create "Unit Tests for Requirements":
   ...
   **Category Structure** - Group items by requirement quality dimensions:
   ...
   - **Acceptance Criteria Quality** (Are success criteria measurable? See AC rules below)
   ...
   **Acceptance Criteria Quality Rules** (D18):
   Every acceptance criterion in the spec MUST be validated against these four rules:
   1. **Observable action** — Does the AC specify a concrete action? (API call, UI interaction, data query, CLI command)
   ...
   - "Does AC [N] define expected output (response code, UI state, data value)? [AC Quality]"
   - "Is AC [N] independently verifiable without bundling with other criteria? [AC Quality]"
   - "Where applicable, does AC [N] include measurable thresholds? [AC Quality]"
...
10. **Report**: Output full path to created checklist, item count, and remind user that each run creates a new file. Summarize:
