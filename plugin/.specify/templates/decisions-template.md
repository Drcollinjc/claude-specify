# {{FEATURE_NAME}} — Decision Trace

## Metadata
- **Feature**: {{FEATURE_NUMBER}}-{{FEATURE_SLUG}}
- **Watermark**: {{WATERMARK}}
- **Created**: {{DATE}}
- **Branch**: {{BRANCH_NAME}}

## Gate Decisions

| Stage | Decision | Rationale | Date |
|-------|----------|-----------|------|

## Adjustments

| What Changed | Why | Impact | Date |
|-------------|-----|--------|------|

## Validation Evidence

### User Story 1: [Title]

| Criterion | Result | Evidence | Date |
|-----------|--------|----------|------|

## Learnings

_Insights captured during implementation._

## Implementation Metrics

<!--
  Structured retro signals — auto-appended during /spec-implement.
  These quantified metrics enable data-driven retrospectives.
-->

### Task Metrics

<!--
  Appended at task completion. One entry per task.
  Format:

  #### T014 [US3] ProspectPipeline
  | Metric | Value |
  |--------|-------|
  | Turns | [count] |
  | Self-corrections | [count and brief description] |
  | Files created | [count] |
  | Files modified | [count] |
-->

_Populated during /spec-implement._

### Gate Metrics

<!--
  Appended at gate completion. One entry per GATE_USn.
  Format:

  #### GATE_US3
  | Metric | Value |
  |--------|-------|
  | Engineering ACs | [pass/total] PASS/FAIL |
  | User Journey steps | [pass/total] PASS/FAIL |
  | Intelligence evals | Structural: [result], Semantic: [score] |
  | Validator model | [model used] |
  | Validator turns | [count] |
-->

_Populated during /spec-implement._

### Feature Summary

<!--
  Appended at /spec-implement completion. One entry per feature.

  | Metric | Value |
  |--------|-------|
  | Total tasks | [count] |
  | Total gates | [count] |
  | Gate first-pass rate | [fraction] |
  | Compaction events | [count] |
  | Self-corrections | [total count] |
  | Eval fixtures created | [count] |
  | Estimated cost | [dollar amount] |
-->

_Populated at /spec-implement completion._

## Issues & Tech Debt

| Issue | Severity | Resolution | Status |
|-------|----------|------------|--------|

## Context Snapshots

_Populated automatically by PreCompact hook before context compaction events._
